...
description: "InfoJobs DevBoard uses a local Ollama instance with the qwen2.5:3b model to stream job summaries — no API keys, no usage costs, fully offline."
---
...
The backend constructs a user prompt from the job's stored fields and sends it to Ollama. A separate `systemPrompt` variable constrains the model to only produce a job summary in Spanish:
