...
The canonical shape of a job record is defined by the TypeScript interfaces below. These types ship with the SQL-backed variant of the project and serve as the authoritative reference for the field names and allowed values used across the whole application.
...
interface Job {
  ...
  description: string  // Short description shown in listing cards
  data: JobData
  content?: JobContent
}
...
<Note>
  The JSON data file and the Zod schema use Spanish field names (`titulo`, `empresa`, `ubicacion`, `descripcion`, `modalidad`, `nivel`). The TypeScript interfaces above use English names — the SQL version of the project maps these fields when migrating data.
</Note>
